...
class Brain6:
	...
	def __init__(self):
		...
		br=Neuron(range(-15,16,1),eyeWTA, None,'br-neuron','linear',None,None)
		self.networks.append([br])

		#BRAIN6 network for sensing touch and setting an colorintensity threshold on the center eye
		touchNetwork=[]
		touchThreshold=0.0
		#eta=.1
		bias=1
		...
		self.networks.append(touchNetwork)
	...
	def run(self,charge,touch,eye,ear0,ear1,actuators,headangle):
        ...
        #ENDTEST
		brNeuron=self.networks[3][0]
		brNeuron.propagate()
		br=brNeuron.y
	
		#touch network
		touchNetwork=self.networks[4]
		...
		#print touchNeuron.y,ciNeuron.y,andNeuron.y		
